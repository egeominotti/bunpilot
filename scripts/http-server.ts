// Simple HTTP server for bunpm2 testing
// Responds with worker info on every request

const workerId = process.env.BUNPM2_WORKER_ID ?? 'standalone';
const port = parseInt(process.env.BUNPM2_PORT ?? '3000', 10);
const appName = process.env.BUNPM2_APP_NAME ?? 'unknown';
...
// Notify master that we're ready (if running under bunpm2)
