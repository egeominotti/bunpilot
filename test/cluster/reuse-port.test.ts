// ---------------------------------------------------------------------------
// bunpilot – Unit Tests for ReusePortCluster
// ---------------------------------------------------------------------------
...
describe('ReusePortCluster', () => {
  ...
  describe('getWorkerEnv', () => {
    test('returns BUNPILOT_PORT as a string of the given port', () => {
      const env = cluster.getWorkerEnv(1, 3000);
      expect(env.BUNPILOT_PORT).toBe('3000');
    });

    test('returns BUNPILOT_REUSE_PORT set to "1"', () => {
      const env = cluster.getWorkerEnv(1, 3000);
      expect(env.BUNPILOT_REUSE_PORT).toBe('1');
    });
    ...
    test('uses the correct port for different values', () => {
      ...
      expect(env.BUNPILOT_PORT).toBe('9090');
    });
  });
